...
class TeamDetailView(AccountPermissionsMixin, View):
    def test_team_create_quota(
        self, request: HttpRequest, team_pk: typing.Optional[int]
    ) -> typing.Optional[HttpResponse]:
        if team_pk is None and resource_limit_met(self.account, QuotaName.MAX_TEAMS):
            upgrade_message = get_subscription_upgrade_text(
                self.has_permission(AccountPermissionType.ADMINISTER), self.account
            )

            messages.error(
                request,
                "This account has reached the maximum number of teams allowed by its current "
                "subscription. {}".format(upgrade_message),
                extra_tags="safe",
            )
    ...
    def post(
        ...
        if request.POST.get("action") == "remove_team" and team_pk:
            ...
            messages.success(
                request,
                "Team <em>{}</em> was deleted successfully.".format(escape(team.name)),
                extra_tags="safe",
            )
            return redirect("account_team_list", self.account.name)
                ...
                try:
                    ...
                    if action == "add_member":
                        if user not in team.members.all():
                            ...
                            messages.success(
                                request,
                                "{} was added to team {}.".format(user.username, team),
                            )
        ...
        if request.POST.get("action") == "add_project":
            ...
            if project_agent_role.agent_type != AgentType.TEAM:
                return JsonResponse(
                    {
                        "success": False,
                        "message": "The role you trying to update is not for a team.",
                    }
                )
            ...
            if project_agent_role.team != team:
                return JsonResponse(
                    {
                        "success": False,
                        "message": "The role you trying to update is not for this team.",
                    }
                )
