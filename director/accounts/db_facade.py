...
class AccountFetchResult(typing.NamedTuple):
    """
    Represents the result of an account fetch for a `User`.

    Properties it includes are:
...
def fetch_team_for_account(account: Account, team_pk: typing.Optional[int]) -> Team:
    """
    Get Team with pk=team_pk. If team_pk is None a new Team is instantiated and returned (not saved to DB).

    The function checks if the Team belongs to the passed in Account and if not raises PermissionDenied. It is assumed
