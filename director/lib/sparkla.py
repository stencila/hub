...
def generate_jwt(
    ...
    url: str,
    user_id: str,
    environment_id: SparklaEnvironment = DEFAULT_ENVIRONMENT,
    project: typing.Optional[Project] = None,
    project_id_override: typing.Optional[str] = None,
) -> str:
    ...
    limits: typing.Dict[str, typing.Any] = {"url": url, "userId": user_id}

    if project_id_override:
        limits["projectId"] = project_id_override
    elif project:
        account = project.account
        resource_limits = account_resource_limit_multiple(
            account,
            {
        ...
        if resource_limits[QuotaName.MAX_CLIENTS_PER_SESSION] != -1:
            limits["clientsPerSession"] = resource_limits[
                QuotaName.MAX_CLIENTS_PER_SESSION
            ]

        if resource_limits[QuotaName.SESSION_MEMORY_LIMIT] != -1:
            software_session.memoryLimit = resource_limits[
                QuotaName.SESSION_MEMORY_LIMIT
            ]
        ...
        project_dir = os.path.join(
            settings.SPARKLA_PROJECT_ROOT, "{}".format(project.pk)
        )
        software_session.volumeMounts = [
            stencila.schema.types.VolumeMount("/project", mountSource=project_dir)
        ]
    ...
    return [
        {
            "url": url,
            "jwt": generate_jwt(
                jwt_secret, url, user_id, environment_id, project, project_id_override
            ),
        }
