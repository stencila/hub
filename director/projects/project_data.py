...
FILTER_OPTIONS = (
    ...
    FilterOption(
        "shared",
        "Shared with me",
        "Projects to which you or one of your Teams have access.",
    ),
...
def get_projects(
    user: User, filter_key: typing.Optional[str], limit: typing.Optional[int] = None
) -> ProjectFetchResult:
    if not user.is_authenticated:
        ...
        if filter_key == "account":
            ...
            project_ids = (
                ProjectAgentRole.filter_with_user_teams(user=user)
                .exclude(project__creator=user)
    ...
    return ProjectFetchResult(
        FILTER_LOOKUP[filter_key], projects, available_results, remaining_results
    )
