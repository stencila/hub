...
urlpatterns = [
                ...
                path(
                    ...
                    include(
                        [
                            path(
                                "manifest",
                                ProjectHostManifestView.as_view(),
                                {"version": 0},
                            ),
                            re_path(
                                r"^environ/(?P<environ>.*)",
                                ProjectHostSessionsView.as_view(api_version=0),
                                name="session_start_v0",
                            ),
                            path(
                                "session-queue",
                                ProjectSessionRequestView.as_view(api_version=0),
                                name="session_queue_v0",
                            ),
                        ]
                    ),
                ),
                path(
                    ...
                    include(
                        [
                            ...
                            re_path(
                                r"^sessions/(?P<environ>.*)",
                                ProjectHostSessionsView.as_view(api_version=1),
                                name="session_start_v1",
                            ),
                            path(
                                "session-queue",
                                ProjectSessionRequestView.as_view(api_version=1),
                                name="session_queue_v1",
                            ),
                        ]
                    ),
                ),
    ...
    path(
        "<str:token>/session-setup/<path:environ>",
        ProjectSessionSetupView.as_view(),
...
if settings.FEATURES["PROJECT_SESSION_SETTINGS"]:
    urlpatterns.append(
        path(
            "<int:pk>/settings/sessions",
            ProjectSettingsSessionsView.as_view(),
            name="project_settings_sessions",
        )
    )
