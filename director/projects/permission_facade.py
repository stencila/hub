...
from projects.permission_models import ProjectRole, ProjectAgentRole, ProjectPermissionType, ProjectPermission
...
def fetch_project_for_user(user: AbstractUser, project_pk: typing.Optional[typing.Union[str, int]] = None,
    ...
    if not project:
        if account_name or project_name:
            ...
            project = get_object_or_404(Project, account__name=account_name, name=project_name)
        elif project_pk:
            project = get_object_or_404(Project, pk=project_pk)

    if not project:
        raise ValueError('Either pk or account_name and project_name or project must be provided.')
    ...
    if user.is_authenticated and user_is_account_admin(user, project.account):
        ...
        own_permission = ProjectPermission.objects.get(type=ProjectPermissionType.OWN.value)
