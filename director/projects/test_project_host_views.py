...
from projects.views.project_host_views import (
...
ob_get_path = "projects.views.project_host_views.get_object_or_404"


class ProjectHostSessionsViewTests(TestCase):
    def setUp(self):
        self.view = ProjectHostSessionsView()
        self.view.session_facade = mock.MagicMock(
            spec=CloudSessionFacade, name="CloudSessionFacade"
        )
    ...
    def test_project_access_denied(self):
        ...
        with mock.patch(
            ob_get_path, return_value=mock.MagicMock(spec=Project)
        ) as mock_fetch:
    ...
    def test_get_session_url_from_request_session_not_set(self):
        ...
        url = self.view.get_session_from_request_session(
            self.request, "CLOUD_SESSION_URL_token_environ"
        )
        self.assertIsNone(url)

    @mock.patch("projects.views.project_host_views.Session", spec=Session)
    def test_get_session_url_from_request_session_no_session_object(
    ...
    @mock.patch("projects.views.project_host_views.Session", spec=Session)
    def test_get_session_url_from_request_session_stopped_session(
        self, mock_session_class
    ):
        ...
        mock_session.stopped = timezone.now() - timedelta(
            seconds=300
        )  # just needs to be any time in the past
    ...
    def test_get_session_url_from_request_session_success(self, mock_session_class):
        ...
        session = self.view.get_session_from_request_session(
            self.request, "CLOUD_SESSION_URL_token_environ"
        )
    ...
    @mock.patch("projects.views.project_host_views.SessionRequest", spec=SessionRequest)
    def test_get_session_request_to_use(self, mock_session_request_class):
        ...
        with that pk.
        """
        mock_session_request_class.objects = mock.MagicMock(
            name="SessionRequest.objects"
        )

        self.request.session = {
            "SESSION_REQUEST_NEXT_token": True,
        ...
        self.assertEqual(
            session_request, mock_session_request_class.objects.get.return_value
        )

    @mock.patch("projects.views.project_host_views.redirect")
    def test_create_session_request(self, mock_redirect):
        ...
        response = self.view.create_session_request(
            self.request, self.token, self.environ
        )
        ...
        self.assertEqual(
            self.request.session["SESSION_REQUEST_ID_token"],
    ...
    def test_create_session(self):
        ...
        session_request_to_use = mock.MagicMock(
            spec=SessionRequest, name="session_request_to_use"
        )
        session = self.view.create_session(
            self.request, self.environ, session_key, session_request_to_use
        )

        self.assertEqual(
            session.url, self.view.session_facade.create_session.return_value.url
        )
        self.assertEqual(self.request.session[session_key], session.url)

    @mock.patch("projects.views.project_host_views.JsonResponse")
    def test_generate_response_existing_session_url(self, mock_json_response_class):
        ...
        self.view.get_session_request_to_use = mock.MagicMock(
            name="get_session_request_to_use"
        )
        self.view.create_session = mock.MagicMock(name="create_session")

        response = self.view.generate_response(
            self.request,
            self.environ,
            "session_key",
            self.token,
        ...
        self.view.session_facade.generate_external_location.assert_called_with(
            mock_session, extra_auth_parameters
        )
    ...
    def test_generate_response_create_session_success(self, mock_json_response_class):
        ...
        self.view.create_session = mock.MagicMock(name="create_session")
        self.view.create_session_request = mock.MagicMock(
            name="self.create_session_request"
        )
        extra_auth_parameters = mock.MagicMock(spec=dict)
        ...
        self.view.create_session.assert_called_with(
            self.request,
            self.environ,
            "session_key",
    ...
    def test_generate_response_create_session_exception(self):
        ...
        self.view.create_session = mock.MagicMock(
            name="create_session", side_effect=ActiveSessionsExceededException
        )
        self.view.create_session_request = mock.MagicMock(
            name="self.create_session_request"
        )
        ...
        response = self.view.generate_response(
            self.request, self.environ, "session_key", self.token, None
        )

        self.assertEqual(response, self.view.create_session_request.return_value)
        self.view.create_session_request.assert_called_with(
            self.request, self.token, self.environ
        )
    ...
    def test_post(self, mock_get_object_or_404):
        ...
        self.view.get_session_from_request_session = mock.MagicMock(
            name="get_session_from_request_session"
        )
        self.view.generate_response = mock.MagicMock(name="generate_response")
        self.view.project_fetch_result = mock.MagicMock(
            spec=ProjectFetchResult, name="project_fetch_result"
        )
        ...
        self.view.get_session_from_request_session.assert_called_with(
            self.request, "CLOUD_SESSION_URL_token_environ"
        )

        self.view.generate_response.assert_called_with(
            self.request,
            self.environ,
...
class TestProjectSessionRequestView(TestCase):
    ...
    def test_get_session_request_success(self, session_request_class):
        ...
        """
        self.request.session = {"SESSION_REQUEST_ID_token": 100}
        ...
        self.assertEqual(
            session_request, session_request_class.objects.get.return_value
        )
    ...
    def test_session_can_start_no_sessions_available(self):
        ...
        self.view.project_has_sessions_available = mock.MagicMock(
            name="project_has_sessions_available", return_value=False
        )

        self.assertFalse(
            self.view.session_can_start(
                self.request, project, self.token, session_request
            )
        )
    ...
    def test_session_can_start_request_not_first_in_queue(self):
        ...
        session_request = mock.MagicMock(spec=SessionRequest, name="SessionRequest")
        self.view.project_has_sessions_available = mock.MagicMock(
            name="project_has_sessions_available", return_value=True
        )
        self.view.get_first_session_request = mock.MagicMock(
            name="get_first_session_request"
        )
    ...
    def test_session_can_start_success(self):
        ...
        self.view.get_first_session_request = mock.MagicMock(
            name="get_first_session_request", return_value=session_request
        )
    ...
    @mock.patch("projects.views.project_host_views.get_object_or_404")
    @mock.patch("projects.views.project_host_views.JsonResponse")
    def test_get_with_invalid_session_request(
        self, mock_json_response_class, mock_get_object
    ):
        project = mock.MagicMock(spec=Project, name="Project")
        ...
        self.assertEqual(response, mock_json_response_class.return_value)
        mock_json_response_class.assert_called_with({"invalid_session": True})

    @mock.patch("projects.views.project_host_views.get_object_or_404")
    @mock.patch("projects.views.project_host_views.JsonResponse")
    @mock.patch("projects.views.project_host_views.settings")
    def test_get_with_success(
        self, mock_settings, mock_json_response_class, mock_get_object
    ):
        ...
        self.view.session_can_start.assert_called_with(
            self.request,
            project,
            self.token,
