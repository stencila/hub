...
from .source_models import SourceAddress, GithubSource, GoogleDocsSource
        ...
        for part in path.parts:
            if re.match(r"^\.+$", part):
                raise ValidationError(
                    'The path must not contain "." or ".." as components.'
                )
...
class GithubSourceForm(ModelFormWithSubmit):
    ...
    def raise_repo_validation_error(repo: str) -> None:
        raise ValidationError(
            '"{}" is not a valid Github repository. A repository (in the format "username/path", or '
    ...
    def clean_repo(self) -> str:
        ...
        repo_match = re.match(
            r"^((github\.com/)|/)?([a-z\d](?:[a-z\d]|-(?=[a-z\d])){0,38})/([\w_-]+)/?$",
            github_url.path,
            re.I,
        )
...
class GoogleDocsSourceForm(ModelFormWithSubmit):
    ...
    def clean_doc_id(self) -> str:
        ...
        source_address = typing.cast(
            SourceAddress,
            GoogleDocsSource.parse_address(doc_id, naked=True, strict=True),
        )
        return source_address.doc_id
