...
class Archiver(object):
    ...
    def generate_project_archive_directory(self) -> str:
        raise NotImplementedError(
            "Subclasses must implement generate_project_archive_directory"
        )
    ...
    def build_archive_paths(self, archive_name: str) -> typing.Tuple[str, str]:
        ...
        if not path_is_in_directory(output_path, output_dir):
            raise OSError(
                "Archive output path is not inside Project's archive directory."
            )

        return output_dir, output_path

    def do_archive(
        self, project_dir: str, archive_name: str, always_create: bool = True
    ) -> str:
...
class ProjectArchiver(Archiver):
    ...
    def generate_archive_name(self, prefix: typing.Optional[str]) -> str:
        ...
        return "{}{}-{}".format(
            prefix, formatted_name, datetime.now().strftime("%Y-%m-%d-%H-%M-%S")
        )
    ...
    def archive_project(self, name_prefix: typing.Optional[str] = None) -> str:
        ...
        event = ProjectEvent.objects.create(
            event_type=ProjectEventType.ARCHIVE.name,
...
class SnapshotArchiver(Archiver):
    ...
    def generate_archive_name(self) -> str:
        ...
        return "snapshot-{}-v{}{}.zip".format(
            self.snapshot.project.name, self.snapshot.version_number, tag
        )

    def archive_snapshot(self) -> str:
        archive_name = self.generate_archive_name()
        project_dir = generate_project_storage_directory(
            self.archive_root, self.snapshot.project
        )
