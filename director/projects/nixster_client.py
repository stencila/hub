...
class NixsterClient(RestClientBase):
    ...
    def start_container(
        self,
        environment_id: str,
        session_parameters: typing.Optional[dict] = None,
        ...
        if session_parameters:
            ...
            if cpu_shares is not None:
                request_data["cpuShares"] = "{}".format(
                    cpu_shares * 1024
                )  # CPU priority is per 1024

            memory_limit = safe_int(session_parameters.get("memory"))
            if memory_limit is not None:
                request_data["memoryLimit"] = "{}".format(
        ...
        result = self.make_request(
            HttpMethod.POST, self.get_full_url("start"), body_data=request_data
        )
        return result["containerId"]

    def stop_container(self, environment_id: str, container_id: str) -> None:
        request_data = {"environmentId": environment_id, "containerId": container_id}
        self.make_request(
            HttpMethod.POST,
            self.get_full_url("stop"),
            extra_jwt_payload={"cid": container_id},
    ...
    def execute(self, environment_id: str, container_id: str, command: str) -> str:
        ...
        response = self.make_request(
            HttpMethod.POST,
            self.get_full_url("execute"),
            extra_jwt_payload={"cid": container_id},
            body_data=request_data,
        )
        return response["output"]

    def generate_attach_context(
        self, environment_id: str, container_id: str
    ) -> SessionAttachContext:
        url_components = list(urlparse(self.get_full_url("interact")))
        url_components[0] = "ws"
        url_components[4] = "containerId={}&environment={}".format(
            quote(container_id), quote(environment_id)
        )
    ...
    def generate_location(
        ...
        path += "{}interact?containerId={}&token={}".format(
            separator, quote(execution_id), token
        )
    ...
    def get_session_info(self, session: Session) -> SessionInformation:
        ...
        if not container_id:
            ...
            )  # If we have no container_id then it must not be running
        ...
        response = self.make_request(
            HttpMethod.POST,
            self.get_full_url("container-status"),
            extra_jwt_payload={"cid": session.execution_id},
            body_data=request_data,
        )
