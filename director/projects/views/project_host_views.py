...
class ProjectHostManifestView(ProjectHostBaseView):
    def get(self, request, token, version=1):
        ...
        manifest = {
            ...
            "stencila": {"package": "hub", "version": "0.1"},
            "environs": [
                # TODO: this should come from the list of environments
                # available for this project.
                {"id": "stencila/core", "name": "stencila/core", "version": "0.1"}
            ],
            "services": [],
        }
...
class CloudClientMixin(object):
    ...
    def setup_cloud_client(self, project: Project) -> None:
        ...
        client_class = {"NIXSTER": NixsterClient}.get(
            settings.EXECUTION_CLIENT, CloudClient
        )
...
class ProjectSessionRequestView(CloudClientMixin, ProjectHostBaseView):
    ...
    ) -> typing.Optional[SessionRequest]:
    ...
    def get_first_session_request(project: Project) -> typing.Optional[SessionRequest]:
        return (
            SessionRequest.objects.filter(project=project).order_by("created").first()
        )
    ...
    def project_has_sessions_available(self, project: Project) -> bool:
        ...
        return (
            project.sessions_concurrent > self.session_facade.get_active_session_count()
        )

    def session_can_start(
        self,
        request: HttpRequest,
        project: Project,
        token: str,
        ...
        request.session[
            SESSION_REQUEST_IS_NEXT_KEY_FORMAT.format(token)
        ] = True  # semaphore to shortcut checks
    ...
    def get(self, request: HttpRequest, token: str) -> HttpResponse:  # type: ignore
        ...
        return JsonResponse(
            {
                "start_session": self.session_can_start(
                    request, project, token, session_request
                )
            }
        )  # TODO: work out what a sensible response is


def generate_project_file_path(
    project: Project, file_path: typing.Optional[str]
) -> typing.Optional[str]:
    if not file_path:
        return None
...
class ProjectHostSessionsView(CloudClientMixin, ProjectHostBaseView):
    ...
    def get_session_from_request_session(
    ...
    @staticmethod
    def get_session_request_to_use(
        request: HttpRequest, token: str
    ) -> typing.Optional[SessionRequest]:
        ...
        session_request_key = SESSION_REQUEST_SESSION_KEY_FORMAT.format(token)
        try:
            session_request_to_use = SessionRequest.objects.get(
                pk=request.session[session_request_key]
            )
        except SessionRequest.DoesNotExist:
    ...
    def create_session_request(
        self, request: HttpRequest, token: str, environ: str
    ) -> HttpResponse:
        session_request = self.session_facade.create_session_request(environ)
        request.session[
            SESSION_REQUEST_SESSION_KEY_FORMAT.format(token)
        ] = session_request.pk
    ...
    def create_session(
        ...
        environ: str,
        session_key: str,
        session_request_to_use: typing.Optional[SessionRequest],
    ) -> Session:
        self.session_facade.expire_stale_session_requests()
    ...
    def generate_response(
        ...
        if not session:
            ...
            try:
                session = self.create_session(
                    request, environ, session_key, session_request_to_use
                )
        ...
        return JsonResponse(
            {
                "location": self.session_facade.generate_external_location(
                    session, authorization_extra_parameters
                ).to_dict(),
    ...
    def post(self, request: HttpRequest, token: str, environ: str) -> HttpResponse:  # type: ignore
        ...
        if project.key and request.POST.get("key") != project.key:
            raise PermissionDenied(
                "The key in the POST request does not match that of the Project"
            )
        ...
        highest_permission_name = (
            highest_permission.value if highest_permission else None
        )
        ...
        return self.generate_response(
            request, environ, session_key, token, session, extra_auth_params
        )
