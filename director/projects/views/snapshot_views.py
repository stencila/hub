...
class SnapshotView(ProjectPermissionsMixin, View):
    ...
    ) -> typing.Tuple[Snapshot, typing.Optional[str]]:
...
class FileBrowserView(SnapshotView):
    ...
    def get(  # type: ignore
        ...
        return render(
            request,
            "projects/snapshot_files.html",
            self.get_render_context(
                {
                    "page_title": "Snapshot {} Files".format(
                        snapshot.tag or snapshot.version_number
                    ),
...
class ContentView(SnapshotView):
    ...
    def get(  # type: ignore
        ...
        return render(
            ...
            {"file_path": path, "snapshot": snapshot, "file_content": file_content},
        )
...
class PreviewView(ConverterMixin, SnapshotView):
    ...
    def get(  # type: ignore
        ...
        pi, created = PublishedItem.objects.get_or_create(
            project=self.project, snapshot=snapshot, source_path=path
        )

        published_path = utf8_path_join(
            generate_snapshot_publish_directory(settings.STORAGE_DIR, snapshot),
            "{}.html".format(pi.pk),
        )
        ...
        if created or not pi.path:
            ...
            try:
                ...
                messages.error(
                    request, error_format.format(filename), extra_tags="safe"
                )
...
class PreviewMediaView(SnapshotView):
    def get(  # type: ignore
        ...
        snapshot, _ = self.get_snapshot_and_path(
            request, account_name, project_name, version
        )
