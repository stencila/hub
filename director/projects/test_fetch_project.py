...
class TestFetchProjectForUser(TestCase):
    def setUp(self):
        ...
        self.project1 = Project.objects.create(
            account=account, creator=self.user1, name="project1"
        )
        self.project2 = Project.objects.create(
            account=account, creator=self.user2, name="project2"
        )
        ...
        AccountUserRole.objects.create(
            user=self.user2, account=account, role=account_member_role
        )
        AccountUserRole.objects.create(
            user=self.admin, account=account, role=account_admin_role
        )
    ...
    def test_member_project_fetch(self):
        ...
        fetch_result_1_allowed = fetch_project_for_user(
            self.user1, account_name="account", project_name="project1"
        )
        self.assertIn(
            ProjectPermissionType.OWN, fetch_result_1_allowed.agent_permissions
        )

        fetch_result_1_disallowed = fetch_project_for_user(
            self.user1, account_name="account", project_name="project2"
        )
    ...
    def test_admin_project_fetch(self):
        ...
        admin_allowed_1 = fetch_project_for_user(
            self.admin, account_name="account", project_name="project1"
        )

        self.assertIn(ProjectPermissionType.OWN, admin_allowed_1.agent_permissions)

        admin_allowed_2 = fetch_project_for_user(
            self.admin, account_name="account", project_name="project2"
        )
        self.assertIn(ProjectPermissionType.OWN, admin_allowed_2.agent_permissions)

        admin_disallowed_1 = fetch_project_for_user(
            self.admin, account_name="account2", project_name="project3"
        )
