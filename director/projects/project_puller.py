...
class ProjectSourcePuller(object):
    ...
    def pull_directory(
        self, sub_directory: typing.Optional[str] = None, only_file_sources: bool = True
    ) -> None:
        """
        ...
        dir_list = list_project_virtual_directory(
            self.project, sub_directory, self.authentication, only_file_sources
        )
        ...
        for entry in dir_list:
            ...
            if entry.type in (
                ...
                scf = make_source_content_facade(
                    self.request.user, entry.path, entry.source, self.project
                )
                ...
                if scf.error_exists:
                    for message in scf.message_iterator():
                        messages.add_message(
                            self.request, message.level, message.message
                        )
                    return

        directory_entries = filter(
            lambda e: e.type
            in (DirectoryEntryType.DIRECTORY, DirectoryEntryType.LINKED_SOURCE),
            dir_list,
        )
