...
class RestClientBase(object):
    class_id: typing.Optional[
        str
    ] = None  # NOQA flake8 is dumb and thinks I am defining a class here so gives E701
    server_host: str
    server_proxy_path: str
    jwt_secret: str

    def __init__(
        self, server_host: str, server_proxy_path: str, jwt_secret: str
    ) -> None:
    ...
    def get_authorization_header(
        self, extra_payload: typing.Optional[dict] = None
    ) -> typing.Dict[str, str]:
        return {
    ...
    def make_request(
        ...
        method: HttpMethod,
        url: str,
        extra_jwt_payload: typing.Optional[dict] = None,
        body_data: typing.Optional[dict] = None,
    ) -> dict:
        # TODO: add `SessionParameters` to the POST body (currently they won't do anything anyway)
        response = requests.request(
            method.value,
            url,
            headers=self.get_authorization_header(extra_jwt_payload),
            json=body_data,
            timeout=25,
        )
        ...
        try:
            ...
            if (
                response_body == ""
            ):  # to satisfy flake8 unused variable, but we can set a breakpoint here
                pass
            raise e
